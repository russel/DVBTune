...
pub struct ChannelsData {
    file_ptr: FilePtr,
    frontend_parameters_ptr: FrontendParametersPtr,
}

impl ChannelsData {

    /// Create a `ChannelData` instance given a `FilePtr` instance and a `FrontendParametersPtr` instance.
    fn new(file_ptr: FilePtr, frontend_parameters_ptr: FrontendParametersPtr) -> Result<ChannelsData, ()> {
    ...
    pub fn write(&self, output_path: &Path) -> bool {
        if self.file_ptr.is_null() {
            ...
            unsafe {
                if dvbv5_sys::dvb_write_file_format(
                    CString::new(output_path.to_str().unwrap()).unwrap().as_ptr(),
                    self.file_ptr.get_c_ptr(),
                    (*self.frontend_parameters_ptr.get_c_ptr()).current_sys,
                    dvbv5_sys::dvb_file_formats_FILE_DVBV5) == 0 {
                    self.frontend_parameters_ptr.log(dvbv5_sys::LOG_INFO, &format!("\nWrote virtual channels file to: {}", output_path.display()));
                    true
                } else {
    ...
    pub fn scan(
        ...
        match FrontendParametersPtr::new(&frontend_id,None, None) {
            Ok(frontend_parameters) => {
                ...
                Ok(ChannelsData::new(FilePtr::new_from_dvb_file_ptr(channels_file).unwrap(), frontend_parameters).unwrap())
            },
            Err(e) => Err(e),
        }
    }
